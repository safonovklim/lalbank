module Api::V1
  class CardController < ApiController
    ...
    def index
      cards = BankAccount
                  .select("cards.id, cards.card_number, bank_accounts.amount, bank_accounts.currency")
                  .where('bank_accounts.client_id = ?', @current_client[:id])
                  .where('cards.status = 0 or cards.status = 1')
