module Api::V1
  class AnalysisController < ApiController
    ...
    def index
      ...
      if month >= 1 && month <= 12
        if year >= @current_client[:created_at].year.to_i && year <= Time.now.year.to_i
          if @current_client[:created_at].year.to_i == Time.now.year.to_i
            # check for month
            if month >= @current_client[:created_at].month.to_i
              # proceed
    ...
    private
      ...
      def render_result(allowed = false, analysis = nil)
        allowed_from = "#{@current_client[:created_at].year}-#{@current_client[:created_at].month}"
